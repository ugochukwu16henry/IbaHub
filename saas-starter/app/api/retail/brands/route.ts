...
import { retailBrands } from '@/lib/db/schema';
...
export async function GET() {
  try {
    ...
    const rows = await db
      .select()
      .from(retailBrands)
      .where(eq(retailBrands.teamId, team.id))
      .orderBy(desc(retailBrands.createdAt));
...
export async function POST(request: Request) {
  try {
    ...
    const [row] = await db.insert(retailBrands).values({ teamId: team.id, ...parsed.data }).returning();
