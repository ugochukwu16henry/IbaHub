...
export async function GET(_: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [row] = await db
      .select()
      .from(retailBrands)
      .where(and(eq(retailBrands.id, id), eq(retailBrands.teamId, team.id)))
      .limit(1);
...
export async function DELETE(_: Request, context: { params: Promise<{ id: string }> }) {
  try {
    const { team } = await requireRetailContext({ ownerWrite: true });
    const id = parseId((await context.params).id);
    await db.delete(retailBrands).where(and(eq(retailBrands.id, id), eq(retailBrands.teamId, team.id)));
